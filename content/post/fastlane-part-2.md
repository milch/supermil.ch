...
<!-- add link to last post [post](link) -->
As we saw in my last post, fastlane is mostly controlled through a configuration file (the `Fastfile`) located in the `fastlane/` subfolder in your project directory.  
You can create this folder and the `Fastfile` manually if you want, but for the purpose of this guide we will let fastlane do the work for us.  
...
fastlane will proceed to ask you some questions about your project including your Apple ID and password.  
Note that fastlane uses the terms "bundle ID" and "app ID" interchangeably.
  
As soon as you confirm your app's bundle ID, a couple things will happen:  
<!-- you could add links here to the tools -->
- If your app doesn't exist on the Apple Developer portal or on iTunes Connect `produce` will create it
- Any existing metadata from iTunes Connect will be downloaded by `deliver` 
...
Note that fastlane will always save your credentials in your system's keychain after you've entered them for the first time.  
If you do not want that, you can also set your username and password by setting the `FASTLANE_USER` and `FASTLANE_PASSWORD` environment variables. 
...
Something that we haven't discussed yet when I was talking about the DSL is the `platform` block:  
it simply scopes all the lanes it contains to a single platform.  
...
<!-- removed word repetition 'this', not sure if 'it' is the best replacement though -->
...
Next, we want to setup fastlane to be able to manipulate version information.  
We can get the information how to do this [directly from Apple](https://developer.apple.com/library/content/qa/qa1827/_index.html), but it is a straightforward process:
<!-- I'm confused here, are you suggesting a simpler way than apple, or just describing what there is on the link you provide? In the second case, I'd remove the link and write "This is a straightforward process:" -->
...
To get automatic screenshot creation you need to use Xcode UI tests.  
If you aren't using them already, you can simply add them by clicking on your project file in Xcode and clicking on `Editor > New Target...` in the menu bar.  
Scroll down until you find `iOS UI Testing Bundle` and add it to your project. 

We can let fastlane set everything up for us: 

```bash
fastlane snapshot init
```

This will create two new files in your fastlane directory - the `Snapfile` and `SnapshotHelper.swift`.  
Everything you need to know about the `Snapfile` is explained inside, so take a look at it.  
You should set all the languages that you want screenshots for as well as the list of devices by uncommenting the respective lines. 
...
<!-- Here should be a separation of some kind. Maybe a subheader (which would make it necessary to add a subheader to the above section as well, or just making the sentence bold (nope, doesn't work), so add a subheader, like "Setup" and "Execution", I don't really like them, but you get what I mean -->
The easiest way to do this is to open your UI tests, create a new function `testTakeScreenshots` and then use Xcode's record function to navigate to the screens you want to take screenshots of. Then you simply insert the line `snapshot(<filename>)` for each screenshot you want to take:
...
<!-- Does your audience know what an SCM is? I have no idea (but I don't belong to your audience -->
...
It is also a good idea to automatically increment your build number each time you do a build.  
...
### Wrapping up
<!-- I've changed the order of the paragraphs, the preview of the next post should be after the actual wrapping up part. Not sure if this makes sense in this order though. I belive it is possible to create a better transition, something that makes more apparent in the last sentence that the remaining parameters are related to code signing. 
Because you say there is one last problem, and then, boom, you tell them that you're going to explain how -->
If all your code signing works already and you are using a fairly standard project setup, then it is likely that you can now release new versions of your app using fastlane.  
If this is not the case for you, you can find out the parameters of each action by running `fastlane action <action_name>` - for example, some actions might require you to set the correct Scheme or Target of your Xcode project before they will work. 

In the next post, we will look at how to fix code signing by using match. Unfortunately code signing is such a complex topic and this post is already too long. 
